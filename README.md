...
完整格式能力的 Svelte 集成包，已内置 `@file-viewer/preset-all`，无需重复安装 preset。要覆盖所有 Worker / WASM 格式，请按下方“Full 包快速开始”发布 `<部署基址>/file-viewer/` 运行时资源。

```bash
npm install @file-viewer/svelte-full
```

包根入口的默认导出固定为已装配完整 preset 的 Svelte 组件；action 与底层 controller helper 使用明确子路径：
...
`@file-viewer/svelte-full` 已内置 `@file-viewer/preset-all` 并默认启用完整 renderer 矩阵。不要再安装或传入 `preset-office`、`preset-all` 或单独 renderer。
...
`@file-viewer/svelte-full` 默认把 PDF、DOCX、PPT、PPTX、Excel、CAD、Typst、Draw.io、SQLite 和 Archive 资产指向部署基址下的 `file-viewer/`（根部署即 `/file-viewer/`）。运行随包安装的同版本 `file-viewer-copy-assets` 并发布其产物后，不需要逐项手写资源 URL。
