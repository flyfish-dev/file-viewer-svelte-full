...
Full-format Svelte package with `@file-viewer/preset-all` built in; do not install another preset. Follow “Full Package Quick Start” below to publish the `<deployment-base>/file-viewer/` runtime assets required by every Worker/WASM format.

```bash
npm install @file-viewer/svelte-full
```

The package-root default export is always the Svelte component with the complete preset already applied. Use explicit subpaths for the action and lower-level controller helpers:
